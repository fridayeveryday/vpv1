...
void QPCRP(vector<int>& vec) {
	...
	while (n < vec.size()) {
		...
		n += 10;
		if (results.size() > 1 && results[results.size() - 1].time > 0 && results[results.size() - 2].time > 0) {
			cout << "n-1 = " << results[results.size() - 2].n << " time = " << results[results.size() - 2].time << " нс";
			cout << endl;
			cout << "n   = " << results[results.size() - 1].n << " time = " << results[results.size() - 1].time << " нс";
			cout << endl;
...
int main()
{
	...
	cout << "menu: 0 - сделать все, 1 - фибоначчи, 2 - разрешающая способность\n";
	int menu;
	cin >> menu;
	if (menu == 1 || menu == 0) {
		int n = 10;
